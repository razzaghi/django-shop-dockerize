...
class Product(models.Model):
    ...
    def total_discount(self):
        return int(min(100, self.off_percent + (100 - self.off_percent) * (
                self.discount_percent + self.coupon_off_percent) / 100))
...
class User_Functions:
    ...
    def get_or_register_user(ip, session_key):
        ...
        if user_:
            ...
            return CustomUser.objects.create_user(username=f'{session_key}_{ip}'.replace('.', '_'), ip=ip,
...
class ActionsLog(models.Model):
    ...
    def get_action_status(request, _product, _action_type, _value):
        ...
        if _user:
            ...
            res = ActionsLog.objects.filter(session_key=_session_key, ip=_ip, product_id=_product.id,
                                            action=_action_type.value).last()
        if res:
